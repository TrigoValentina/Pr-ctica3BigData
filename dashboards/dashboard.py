...
# Si está autenticado, decidir qué dashboard mostrar según el rol
rol = st.session_state.get("user_role", "sin rol")
    ...
    if df.empty:
        ...
        if df_sonido.empty:
            ...
            fig_evolucion = crear_grafico_evolucion_temporal(
                df_sonido, 'LAeq', "Evolución del Nivel de Sonido Promedio (dB)", color_by="Address"
            )
            st.plotly_chart(fig_evolucion, use_container_width=True)
 
            # 1.2 Nivel de Sonido Promedio por Ubicación
            st.markdown("---")
            st.markdown("### 1.2 Nivel de Sonido Promedio por Ubicación")
            fig_barras = crear_grafico_barras_promedio(
                df_sonido, "Address", "LAeq", "Nivel de Sonido Promedio (LAeq) por Ubicación"
            )
            st.plotly_chart(fig_barras, use_container_width=True)
 
            # 1.3 Distribución del Nivel de Sonido por Ubicación
            st.markdown("---")
            st.markdown("### 1.3 Distribución del Nivel de Sonido por Ubicación")
            fig_box = crear_boxplot_distribucion(
                df_sonido, "Address", "LAeq", "Distribución del Nivel de Sonido por Ubicación"
            )
            st.plotly_chart(fig_box, use_container_width=True)
 
            # 1.4 Patrón Hora vs Día
            st.markdown("---")
            ...
            # 1.5 Comparación Temporal LAeq, LAI y LAImax
            st.markdown("---")
            st.markdown("### 1.5 Comparación Temporal entre LAeq, LAI y LAImax")
            if {'LAeq','LAI','LAImax'}.issubset(df_sonido.columns):
                fig_comp = px.line(
                    df_sonido.sort_values("time"),
                    x='time',
                    y=['LAeq','LAI','LAImax'],
                    labels={'value':'Nivel (dB)','variable':'Tipo'},
                    title="Comparación Temporal entre LAeq, LAI y LAImax"
                )
                st.plotly_chart(fig_comp, use_container_width=True)
            else:
                st.info("No hay suficientes columnas para este gráfico")
 
            # 1.6 Promedio por Hora
            st.markdown("---")
            st.markdown("### 1.6 Nivel de Sonido Promedio por Hora del Día")
            df_sonido["hora"] = df_sonido["time"].dt.hour
            fig_horas = px.line(
                df_sonido.groupby("hora")["LAeq"].mean().reset_index(),
                x="hora", y="LAeq", markers=True,
                title="Promedio de LAeq por Hora del Día"
            )
            st.plotly_chart(fig_horas, use_container_width=True)
 
            # 1.7 Promedio por Día
            st.markdown("---")
            st.markdown("### 1.7 Nivel de Sonido Promedio por Día")
            df_sonido["fecha"] = df_sonido["time"].dt.date
            fig_dia = px.line(
                df_sonido.groupby("fecha")["LAeq"].mean().reset_index(),
                x="fecha", y="LAeq", markers=True,
                title="Promedio de LAeq por Día"
            )
            st.plotly_chart(fig_dia, use_container_width=True)
 
            # 1.8 Distribución de Ruido por Rango Acústico
            st.markdown("---")
            st.markdown("### 1.8 Distribución de Ruido por Rango Acústico")
            def clasificar_ruido(val):
                if val < 55: return "Bajo"
                if val < 70: return "Moderado"
                if val < 85: return "Alto"
                return "Muy Alto"
            df_sonido["rango_ruido"] = df_sonido["LAeq"].apply(clasificar_ruido)
            fig_rangos = px.pie(df_sonido, names="rango_ruido", title="Porcentaje de Tiempo por Nivel de Ruido")
